...
- **Voice System**:
    ...
    - Callback contract: page-level `onTranscription` callbacks return `true` if handled (prevents fallthrough to global nav), `false` otherwise. All pages use `useFocusEffect(useCallback(...))` (not `useEffect`) to register/unregister voice callbacks — this prevents background stacked screens from overwriting the active screen's callback.
    - Reader is section-based (not page-based): all sections shown in one scrollable view. Word-by-word highlighting during TTS narration (Spotify lyrics style) with auto-scroll. Controls: skip-prev, play/pause, skip-next. `speakTextWithProgress()` provides real-time audio progress for word highlighting estimation.
    - **Pre-computed summaries**: Book summaries are pre-generated and stored in `data.ts` (fields: `summary` for full book, `pageSummaries[]` for per-page). The "Summarize" command instantly shows both full book + current page summary with zero API latency. No Azure OpenAI call needed — summaries are served directly from local data. This architecture mirrors production where summaries would be pre-generated at book upload time via Azure Functions pipeline.
